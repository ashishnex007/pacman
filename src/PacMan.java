...
public class PacMan extends JPanel implements ActionListener, KeyListener{
        ...
        for(int r = 0; r < rowCount; r++){
            for(int c = 0; c < columnCount; c++){
                ...
                if(tileMapChar == 'X') { // * wall
                    ...
                    Block blueGhost = new Block(orangeGhostImage, x, y, tileSize, tileSize);
                    ghosts.add(blueGhost);
                }
                else if(tileMapChar == 'r'){ // * red ghost
                    Block blueGhost = new Block(redGhostImage, x, y, tileSize, tileSize);
                    ghosts.add(blueGhost);
                }
                else if(tileMapChar == 'p'){ // * pink ghost
                    Block blueGhost = new Block(pinkGhostImage, x, y, tileSize, tileSize);
                    ghosts.add(blueGhost);
                }
                else if(tileMapChar == 'P'){ // * PACMAN ITSELF
    ...
    public void draw(Graphics g){
        ...
        for(Block ghost: ghosts){
            // g.drawImage(ghost.image, ghost.x, ghost.y, ghost.width, ghost.height, getFocusCycleRootAncestor());
        ...
        if(gameOver){
            g.drawString("Game Over " + String.valueOf(score), boardWidth/2 - 50, boardHeight/2);
        }else{
    ...
    public void move(){
        ...
        if(foods.isEmpty()){ // * next level
            score += 100; // * next level bonus
            lives ++; // * next level bonus
            loadMap();
            resetPositions();
        }
    }
